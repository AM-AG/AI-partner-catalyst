...
import { decodeAudioData, base64ToUint8Array } from '../utils/audio-utils';
...
interface LiveSessionDeps {
  ...
  outputTextRef: React.MutableRefObject<string>;
...
export function createLiveSessionCallbacks({
  outputCtx,
  nextPlayTimeRef,
  inputTextRef,
  outputTextRef,
  ...
  return {
    ...
    async onmessage(msg: LiveServerMessage) {
      ...
      if (msg.serverContent?.turnComplete) {
        if (inputTextRef.current) {
          setTranscripts(p => [
            ...p,
            { role: 'user', text: inputTextRef.current }
          ]);
        }
        ...
        if (outputTextRef.current) {
          setTranscripts(p => [
            ...p,
            { role: 'model', text: outputTextRef.current }
          ]);
        }
...
export const start = useCallback(async () => {
    try {
      setError(null);
...
export const sendAsset = useCallback((asset: Attachment) => {
    sessionRef.current?.sendRealtimeInput({
